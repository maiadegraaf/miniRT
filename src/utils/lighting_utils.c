...
/*   Updated: 2022/09/07 15:28:40 by mgraaf        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	check_shadow(t_hittable_lst *world, t_ray light_r, t_lighting *l, float distance)
{
	...
	while (world->prev != NULL)
	{
		...
		{
			l->if_s = true;
			float	intensity = -(distance - light_hit.rec->t - hit_sphere(world->s, *light_hit.r)) / 5000;
			l->shadow = (world->next->color * (1 - intensity)) - (float)(2 * intensity);
...
t_lighting get_point_light(t_point_light light, t_hittable hittable, t_hittable_lst *world)
{
	...
	if (light.diff_power > 0)
	{
		...
		intensity = pow(clamp(dot(hittable.rec->n, h), 0, 1), 50);
